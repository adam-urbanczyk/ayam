 ...
 */
int
ay_pomesht_connecthard(ay_object *o1, ay_object *o2,
		       double offset1, double offset2,
		       ay_object **result)
{
 int ay_status = AY_OK;
 ay_pomesh_object *pm = NULL, *pm1 = NULL, *pm2 = NULL;
 ay_object *newo = NULL;
 ay_point *sp = NULL, *pp = NULL, *qq = NULL, *mm = NULL, *mma = NULL;
 ay_point *p1, *p2, *q1, *q2;
 double *vas1 = NULL, *vas2 = NULL, *m = NULL;
 double *cv = NULL, dist1, dist2;
 unsigned int np1 = 0, np2 = 0, npm = 0;
 unsigned int i, j;
 unsigned int numtris = 0, maxtris;
 int stride = 3, isclosed1 = AY_TRUE, isclosed2 = AY_TRUE;
  ...
  /* make sure, pp is the coarser one */
  if(np1 > np2)
    return ay_pomesht_connecthard(o2, o1, offset2, offset1, result);
  ...
  ay_status = ay_pomesht_mergepoints(pp, np1, qq, np2, &mm, &npm);

  if(ay_status)
    goto cleanup;

  /* make a copy of the hard edge data, so that offsetting (below) does
     not distort the coordinates */
  if(!(m = malloc(npm*stride*sizeof(double))))
    {
      ay_status = AY_EOMEM;
      goto cleanup;
    }
  ...
  for(i = 0; i < npm; i++)
    {
      memcpy(&(m[j]), mm[i].point, stride*sizeof(double));
      mm[i].point = &(m[j]);
      j += stride;
    }

  if(!(mma = malloc(npm*sizeof(ay_point))))
    {
      ay_status = AY_EOMEM;
      goto cleanup;
    }

  memcpy(mma, mm, npm*sizeof(ay_point));
  ...
  /* rotate/shift list of selected points in mm to a good match to o1 */
  ay_pomesht_alignpoints(pp, mm, npm, isclosed1);

  /* generate gap filling triangles */
  maxtris = np1+np2+npm*2;
  if(isclosed2)
    maxtris += 4;
  ...
  while((p1 != &(mm[npm-1])) || (q1 != &(pp[np1-1])) ||
	((p1 == &(mm[npm-1]) && q2 <= &(pp[np1-1]))) ||
	((q1 == &(pp[np1-1]) && p2 <= &(mm[npm-1]))))
    {
      p2 = p1+1;
      q2 = q1+1;
  ...
  /* create triangles to close the strip */
  if(isclosed1)
    {
      p1 = mm;
      p2 = &(mm[npm-1]);
  ...
  /* second strip qq-mm */
  memcpy(mm, mma, npm*sizeof(ay_point));
  ay_pomesht_alignpoints(qq, mm, npm, isclosed2);

  p1 = mm;
  q1 = qq;

  while((p1 != &(mm[npm-1])) || (q1 != &(qq[np2-1])) ||
	((p1 == &(mm[npm-1]) && q2 <= &(qq[np2-1]))) ||
	((q1 == &(qq[np2-1]) && p2 <= &(mm[npm-1]))))
    {
      p2 = p1+1;
      q2 = q1+1;
